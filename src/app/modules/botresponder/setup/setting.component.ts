...
export class SettingComponent implements OnInit {
  ...
  public objectName = "BotResponderMessage"
  isLoading = false;
  record: any = {};
  formOptions: any;
  accountID;
  currentCon;
  formFields: any;
  fields = ['message', 'countdown_to_send'];
  fieldsEdit = ['message', 'countdown_to_send'];
  formFieldsEdit: any;
  public items: any;
  model: any;
  responderID;
  ...
  getSettings() {
    let that = this;
    that.userSvc.appiCall(UserService.actions["BotResponderSettings"], { accountID: this.accountID, responderID: this.responderID }).subscribe(res => {
      if (that.res.status) {
  ...
  updateMsgStatus(event, type) {
    let that = this;
    let form = { accountID: this.accountID, responderID: this.responderID };
    this.settings.data[type] = event.checked;
    form = Object.assign({}, this.settings, form);
    that.userSvc.appiCall(UserService.actions["BotResponderSettingsUpdate"], form).subscribe(res => {
      if (res.status) {
        that.userSvc.showSnak(res);
      }
    });
