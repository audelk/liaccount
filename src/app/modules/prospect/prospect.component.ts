...
export class ProspectComponent implements OnInit {
  ...
  edit(id) {
    ...
    that.opentEditDialog(id, that.formFieldsEdit, that.objectName, 'Update Prospect');
  }
  list() {
    let that = this;
  ...
  openNewDialog() {
    ...
    let dialogRef = this.composeDialog.open(AddnewComponent, {
      width: "720px",
      data: { formFields: this.formFields, opts: { title: 'Add Prospect', buttonText: 'Add' } }
    });
    dialogRef.afterClosed().subscribe(result => {
      if (result != '') {
        that.add(result);
  ...
  update(form, action) {
    ...
    const temp = this.temprecords.filter(function (d) {
      if (d.email.toLowerCase().indexOf(val) !== -1 || d.name.toLowerCase().indexOf(val) !== -1  || d.remarks.toLowerCase().indexOf(val) !== -1)
        return true;
      else
        return false;
