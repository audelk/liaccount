...
export class ConnectionComponent implements OnInit {
  ...
  public objectName = "BotConnectorConnection"
  isLoading = false;
  chatMessages = [];
  type = ''; uID;
  record: any = {};
  ...
  accountID;
  currentCon:any={ID1:0,firstName:undefined};  contactInfo: any = {};
  chatLoading = false;
  selected = [];
  msgToSend = "";
  selectedIDs = [];
  formFieldsConnector: any;
  ...
  list() {
    ...
    that.userSvc.appiCall(UserService.actions[that.objectName + "List"], { accountID: this.accountID, connectorID: this.connectorID }).subscribe(res => {
      ...
      if (that.res.status) {
        ...
        that.importType = res.data.importType;
      }
  ...
  onActivate(event) {
    if (event.type == 'click' && event.column.checkboxable != true) {
      ...
      if (this.currentCon.ID1 != event.row.ID1) {
        ...
        if(event.row.status!='invite_sent'){
